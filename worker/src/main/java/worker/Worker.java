...
class Worker {
  public static void main(String[] args) {
    try {
      Jedis redis = connectToRedis(System.getenv("redis"));
  ...
  static Connection connectToDB() throws SQLException {
    ...
    try {
      ...
      String url = "jdbc:postgresql://" + System.getenv("POSTGRESQL_HOST") + ':' + System.getenv("POSTGRESQL_PORT")
          + "/" + System.getenv("POSTGRESQL_DB");

      while (conn == null) {
        try {
          conn = DriverManager.getConnection(url, System.getenv("POSTGRESQL_USER"),
